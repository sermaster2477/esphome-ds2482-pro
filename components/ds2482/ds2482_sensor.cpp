  ...
  this->set_timeout("read_temp", 800, [this]() {
    ...
    if (this->parent_->reset_1w(this->channel_)) {
      ...
      if ((this->parent_->crc8(data, 8) == data[8]) && !all_zeros) {
        ...
        this->failed_consecutive_read_ = 0; // СБРОС
        success = true;
      } else {
        ESP_LOGW(TAG, "'%s' - CRC Error or All Zeros", this->get_name().c_str());
      }
    } else {
      ESP_LOGW(TAG, "'%s' - No presence pulse", this->get_name().c_str());
    }

    // Если чтение не удалось (success == false)
    if (!success) {
      this->failed_consecutive_read_++;
      this->status_set_warning();
      
      ESP_LOGD(TAG, "'%s' - Consecutive failures: %d/5", this->get_name().c_str(), this->failed_consecutive_read_);

      if (this->failed_consecutive_read_ >= 5) {
        // Проверяем, не NAN ли уже там, чтобы не спамить
        if (!std::isnan(this->state)) {
          ESP_LOGE(TAG, "'%s' - Sensor lost! Sending NAN...", this->get_name().c_str());
